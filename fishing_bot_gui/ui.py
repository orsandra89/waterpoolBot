...
class MyFrame(wx.Frame):
    ...
    def on_start_button_click(self, event):
        if self._process is None or not self._process.is_alive():
            self._process = threading.Thread(target=self.run_function)
            self._process.start()
    ...
    def start_fishing_bot(self, event):
    ...
    def run_function(self):
        delayedresult.startWorker(self.start_fishing_bot, self._function_completed)
